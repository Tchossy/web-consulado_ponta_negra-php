...
const listUsers = async () => {
  const dataUsers = await fetch(
    origin_url + baseURL + 'utenteControllers.php?typeForm=get_utentes'
  )
...
cardForm.addEventListener('submit', async event => {
  ...
  const dataNewUser = await fetch(
    origin_url + baseURL + 'utenteControllers.php?typeForm=create_utente',
    {
      method: 'POST',
      body: dataForm
    }
...
async function confirmDelete(idUtente) {
  await fetch(
    origin_url +
      baseURL +
      'utenteControllers.php?typeForm=delete_utentes&idUtente=' +
      idUtente
  )
  listUsers()
}
...
async function editeUtente(idUtente) {
  const dataUsers = await fetch(
    origin_url +
      baseURL +
      'utenteControllers.php?typeForm=get_utente&idUtente=' +
      idUtente
  )
  ...
  if (response['error']) {
    ...
    document.getElementById('full_name_user_edit').value =
      utenteData.full_name_user
    document.getElementById('email_address_user_edit').value =
      utenteData.email_address_user
    document.getElementById('number_phone_user_edit').value =
      utenteData.number_phone_user
  }
}
...
cardEditForm.addEventListener('submit', async event => {
  ...
  const dataNewUser = await fetch(
    origin_url + baseURL + 'utenteControllers.php?typeForm=edite_utente',
    {
      method: 'POST',
      body: dataEditForm
    }
