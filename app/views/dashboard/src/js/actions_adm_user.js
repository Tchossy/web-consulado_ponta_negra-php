...
const listUsers = async () => {
  const dataUsers = await fetch(
    origin_url + baseURL + 'admUserControllers.php?typeForm=get_utentes'
  )
...
cardForm.addEventListener('submit', async event => {
  ...
  const dataNewUser = await fetch(
    origin_url + baseURL + 'admUserControllers.php?typeForm=create_utente',
    {
      method: 'POST',
      body: dataForm
    }
...
async function editeUtente(idUtente) {
  const dataUsers = await fetch(
    origin_url +
      baseURL +
      'admUserControllers.php?typeForm=get_utente&idUtente=' +
      idUtente
  )
...
cardEditForm.addEventListener('submit', async event => {
  ...
  const dataNewUser = await fetch(
    origin_url + baseURL + 'admUserControllers.php?typeForm=edite_utente',
    {
      method: 'POST',
      body: dataEditForm
    }
